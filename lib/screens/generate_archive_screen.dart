...
class _GenerateArchiveScreenState extends State<GenerateArchiveScreen> {
  ...
  @override
  void dispose() { _topic.dispose(); _api.dispose(); super.dispose(); }

  Future<void> _generate() async {
    if (_topic.text.trim().isEmpty || _loading) return;
    setState(() => _loading = true);
    try { final result = await _api.generateArchive(_topic.text.trim()); if (mounted) setState(() => _result = result); } catch (error) { if (mounted) setState(() => _result = 'Unable to generate archive right now. $error'); } finally { if (mounted) setState(() => _loading = false); }
  }
  ...
  Widget build(BuildContext context) => Scaffold(
        body: SafeArea(
          child: ListView(
            ...
            children: [
              InkWell(onTap: () => Navigator.of(context).pushReplacementNamed('/archive'), child: const Icon(Icons.arrow_back, color: HeritageColors.orange, size: 24)),
