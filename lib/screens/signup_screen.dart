...
class _SignupScreenState extends State<SignupScreen> {
  ...
  @override Widget build(BuildContext context) => Scaffold(body: SafeArea(child: Center(child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 440), child: SingleChildScrollView(child: Column(children: [_hero(), Padding(padding: const EdgeInsets.fromLTRB(24, 0, 24, 48), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [const Text('Create Account', style: TextStyle(color: HeritageColors.cream, fontSize: 24, fontWeight: FontWeight.bold, height: 1.33)), const SizedBox(height: 4), const Text('Preserve your heritage today.', style: TextStyle(color: HeritageColors.brown, fontSize: 14, height: 1.4)), const SizedBox(height: 24), _field(_name, 'Full Name', Icons.person_outline), const SizedBox(height: 16), _passwordField(), const SizedBox(height: 16), _field(_email, 'Email Address', Icons.email_outlined), if (_authMessage.isNotEmpty) Padding(padding: const EdgeInsets.only(top: 12), child: Text(_authMessage, style: const TextStyle(color: HeritageColors.orange, fontSize: 13))), const SizedBox(height: 24), SizedBox(height: 56, child: FilledButton(onPressed: _submitting ? null : _signup, style: FilledButton.styleFrom(backgroundColor: HeritageColors.orange, disabledBackgroundColor: HeritageColors.orange.withOpacity(0.55), foregroundColor: HeritageColors.background, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16))), child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [Text(_submitting ? 'Creating Account...' : 'Begin Journey', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, letterSpacing: 0.5)), const SizedBox(width: 8), const Icon(Icons.arrow_forward, size: 18)]))), const SizedBox(height: 24), Row(children: [Expanded(child: Divider(color: HeritageColors.brown.withOpacity(0.20))), const Padding(padding: EdgeInsets.symmetric(horizontal: 16), child: Text('OR JOIN WITH', style: TextStyle(color: HeritageColors.brown, fontSize: 12, fontWeight: FontWeight.w600, letterSpacing: 0.6))), Expanded(child: Divider(color: HeritageColors.brown.withOpacity(0.20)))]), const SizedBox(height: 24), Row(children: [_social('Google', Icons.g_mobiledata), const SizedBox(width: 16), _social('Apple', Icons.apple)]), const SizedBox(height: 24), Row(mainAxisAlignment: MainAxisAlignment.center, children: [const Text('Already have an account?', style: TextStyle(color: HeritageColors.brown, fontSize: 14)), TextButton(onPressed: () => Navigator.of(context).pushReplacementNamed('/login'), child: const Text('Log in', style: TextStyle(color: HeritageColors.orange, fontSize: 14, fontWeight: FontWeight.bold)))])]))])))));
  ...
  Widget _social(String text, IconData icon) => Expanded(child: SizedBox(height: 56, child: OutlinedButton.icon(onPressed: () {}, icon: Icon(icon, color: HeritageColors.cream), label: Text(text, style: const TextStyle(color: HeritageColors.cream, fontSize: 14, fontWeight: FontWeight.w600)), style: OutlinedButton.styleFrom(backgroundColor: HeritageColors.brown.withOpacity(0.10), side: BorderSide(color: HeritageColors.brown.withOpacity(0.30)), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)))));
}
