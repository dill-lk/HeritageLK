...
class _ContributeScreenState extends State<ContributeScreen> {
      ...
      child: ListView(
        ...
        children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            const Text('Contribute to Archive', style: TextStyle(color: HeritageColors.cream, fontFamily: 'Playfair Display', fontSize: 18, fontWeight: FontWeight.bold)),
            _round(Icons.info_outline, () {}),
          ]),
          const SizedBox(height: 32),
          const Text('SHARE YOUR STORY', style: TextStyle(color: HeritageColors.orange, fontSize: 12, fontWeight: FontWeight.bold, letterSpacing: 2)),
          const SizedBox(height: 4),
          const Text('Add to the Legacy', style: TextStyle(color: HeritageColors.cream, fontFamily: 'Playfair Display', fontSize: 30, fontWeight: FontWeight.bold)),
          const SizedBox(height: 32),
          _field(_title, 'Title of Contribution (e.g. My Grandmother\'s Recipes)', Icons.person_outline),
          ...
          GestureDetector(onTap: () {}, child: Container(height: 150, decoration: BoxDecoration(color: Colors.white.withValues(alpha:0.05), border: Border.all(color: Colors.white.withValues(alpha:0.20)), borderRadius: BorderRadius.circular(16)), child: const Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            SizedBox(height: 4),
            Text('Maximum file size: 50MB', style: TextStyle(color: Color(0x66FFFFFF), fontSize: 12)),
          ]))),
          const SizedBox(height: 16),
          _publicRow(),
          const SizedBox(height: 32),
          SizedBox(height: 56, child: FilledButton(
            ...
            style: FilledButton.styleFrom(backgroundColor: HeritageColors.orange, disabledBackgroundColor: HeritageColors.orange.withValues(alpha:0.50), foregroundColor: HeritageColors.background, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16))),
  ...
  Widget _field(TextEditingController c, String hint, IconData icon) => Container(padding: const EdgeInsets.symmetric(horizontal: 16), decoration: BoxDecoration(color: Colors.white.withValues(alpha:0.05), border: Border.all(color: Colors.white.withValues(alpha:0.10)), borderRadius: BorderRadius.circular(16)), child: TextField(controller: c, style: const TextStyle(color: Colors.white, fontSize: 14), decoration: InputDecoration(icon: Icon(icon, color: const Color(0x66FFFFFF), size: 18), hintText: hint, hintStyle: const TextStyle(color: Color(0x33FFFFFF)), border: InputBorder.none)));

  Widget _categoryField() => Container(
    padding: const EdgeInsets.symmetric(horizontal: 16),
    decoration: BoxDecoration(color: Colors.white.withValues(alpha:0.05), border: Border.all(color: Colors.white.withValues(alpha:0.10)), borderRadius: BorderRadius.circular(16)),
    child: Row(children: [
      const Icon(Icons.folder_outlined, color: Color(0x66FFFFFF), size: 18),
      const SizedBox(width: 12),
      Expanded(child: DropdownButtonHideUnderline(child: DropdownButton<String>(value: _category, dropdownColor: HeritageColors.background, isExpanded: true, style: const TextStyle(color: Colors.white, fontSize: 14), items: const ['Artifacts', 'Oral History', 'Ancient Sites'].map((value) => DropdownMenuItem(value: value, child: Text(value))).toList(), onChanged: (value) => setState(() => _category = value!)))),
    ]),
  );

  Widget _descriptionField() => Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white.withValues(alpha:0.05), border: Border.all(color: Colors.white.withValues(alpha:0.10)), borderRadius: BorderRadius.circular(16)), child: TextField(controller: _description, maxLines: 5, style: const TextStyle(color: Colors.white, fontSize: 14), decoration: const InputDecoration(labelText: 'The Story / Description', labelStyle: TextStyle(color: Color(0x99FFFFFF)), hintText: 'Tell us the historical significance, the origin, or the personal memories associated with this contribution...', hintStyle: TextStyle(color: Color(0x33FFFFFF)), border: InputBorder.none)));

  Widget _publicRow() => Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white.withValues(alpha:0.05), border: Border.all(color: Colors.white.withValues(alpha:0.10)), borderRadius: BorderRadius.circular(16)), child: Row(children: [
