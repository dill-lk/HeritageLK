...
class _ReportAdminScreenState extends State<ReportAdminScreen> {
      ...
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            const SizedBox(height: 24),
            SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: ['all', 'pending', 'in_review', 'resolved', 'rejected'].map((status) => Padding(padding: const EdgeInsets.only(right: 8), child: ChoiceChip(label: Text(status.toUpperCase().replaceAll('_', ' ')), selected: _filter == status, selectedColor: HeritageColors.orange, backgroundColor: Colors.white.withValues(alpha:0.05), labelStyle: TextStyle(color: _filter == status ? HeritageColors.background : const Color(0x99FFFFFF), fontSize: 10, fontWeight: FontWeight.bold), onSelected: (_) => setState(() => _filter = status)))).toList())),
  ...
  Widget _reportCard(DamageReport report) => Container(margin: const EdgeInsets.only(bottom: 12), padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white.withValues(alpha:0.05), border: Border.all(color: Colors.white.withValues(alpha:0.10)), borderRadius: BorderRadius.circular(16)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    Row(children: [
      ...
      TextButton(onPressed: () => _updateStatus(report, 'resolved'), child: const Text('Resolve', style: TextStyle(color: Color(0xFF52B788)))),
      TextButton(onPressed: () => _updateStatus(report, 'rejected'), child: const Text('Reject', style: TextStyle(color: Color(0xFFE76F51)))),
    ]),
  ]));

  Widget _stat(String value, String label, Color color) => Expanded(child: Container(margin: const EdgeInsets.only(right: 6), padding: const EdgeInsets.symmetric(vertical: 12, horizontal: 4), decoration: BoxDecoration(color: color.withValues(alpha:0.06), border: Border.all(color: color.withValues(alpha:0.18)), borderRadius: BorderRadius.circular(12)), child: Column(children: [Text(value, style: TextStyle(color: color, fontSize: 18, fontWeight: FontWeight.bold)), Text(label, style: TextStyle(color: color.withValues(alpha:0.60), fontSize: 8, fontWeight: FontWeight.bold, letterSpacing: 1))])));

  Widget _badge(String status) => Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5), decoration: BoxDecoration(color: HeritageColors.orange.withValues(alpha:0.10), border: Border.all(color: HeritageColors.orange.withValues(alpha:0.30)), borderRadius: BorderRadius.circular(20)), child: Text(status.replaceAll('_', ' '), style: const TextStyle(color: HeritageColors.orange, fontSize: 10, fontWeight: FontWeight.bold)));

  Widget _round(IconData icon, VoidCallback action) => InkWell(onTap: action, borderRadius: BorderRadius.circular(24), child: Container(width: 40, height: 40, decoration: BoxDecoration(color: Colors.white.withValues(alpha:0.10), shape: BoxShape.circle), child: Icon(icon, color: Colors.white, size: 20)));
}
