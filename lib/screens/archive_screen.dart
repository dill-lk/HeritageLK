...
class _ArchiveScreenState extends State<ArchiveScreen> {
      ...
      body: SafeArea(
        child: Stack(children: [
          ListView(padding: const EdgeInsets.fromLTRB(24, 12, 24, 140), children: [
            ...
            TextField(controller: _search, onChanged: (_) => setState(() {}), decoration: InputDecoration(hintText: 'Search history & archives...', hintStyle: const TextStyle(color: Color(0x66FFFFFF), fontSize: 14), prefixIcon: const Icon(Icons.search, color: Color(0x66FFFFFF)), filled: true, fillColor: Colors.white.withValues(opacity:0.05), border: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: BorderSide(color: Colors.white.withValues(opacity:0.10))), enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: BorderSide(color: Colors.white.withValues(opacity:0.10))))),
            const SizedBox(height: 24),
            ...
            if (_error != null) Padding(padding: const EdgeInsets.only(bottom: 16), child: Text('Archive sync unavailable. Showing saved records.', textAlign: TextAlign.center, style: TextStyle(color: HeritageColors.orange.withValues(opacity:0.60), fontSize: 13))),
                    ...
                    child: GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          color: _tab == i ? HeritageColors.orange : Colors.white.withValues(opacity:0.05),
                          borderRadius: BorderRadius.circular(99),
                          boxShadow: _tab == i ? [BoxShadow(color: HeritageColors.orange.withValues(opacity:0.40), blurRadius: 15)] : null,
                        ),
                        child: Text(
                          _tabs[i],
                          style: TextStyle(
          ...
          Positioned(bottom: 108, right: 24, child: GestureDetector(onTap: () => Navigator.of(context).pushNamed('/archive/upload'), child: Container(width: 56, height: 56, decoration: BoxDecoration(color: HeritageColors.orange, shape: BoxShape.circle, boxShadow: [BoxShadow(color: HeritageColors.orange.withValues(opacity:0.40), blurRadius: 20, offset: const Offset(0, 4))]), child: const Icon(Icons.add, color: HeritageColors.background, size: 28)))),
  ...
  Widget _archiveTile(ArchiveRecord item) => InkWell(onTap: () => Navigator.of(context).pushNamed('/archive/${item.id}', arguments: item), borderRadius: BorderRadius.circular(20), child: Container(margin: const EdgeInsets.only(bottom: 16), padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white.withValues(opacity:0.05), border: Border.all(color: Colors.white.withValues(opacity:0.05)), borderRadius: BorderRadius.circular(20)), child: Row(children: [
    ...
    Container(width: 32, height: 32, decoration: BoxDecoration(color: Colors.white.withValues(opacity:0.05), border: Border.all(color: Colors.white.withValues(opacity:0.10)), shape: BoxShape.circle), child: const Icon(Icons.chevron_right, color: Color(0x99FFFFFF), size: 16)),
  ])));
